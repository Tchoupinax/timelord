...
- `TIMELORD_JOB_TIMEOUT`: optional, defaults to `30m`. Maximum duration of a job before the agent kills the whole script process group and reports the exit code `124`. Accepts a Go duration (`45m`, `2h`); `0` disables the timeout. Overridden per script by `#>> Timeout: 12h` in the Timelord header.
